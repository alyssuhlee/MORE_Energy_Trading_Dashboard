...
if scrollable_content:
    duplicated_content = scrollable_content + scrollable_content
    scrolling_container.markdown(
        f"""
        <div class="scrolling-container">
            <div class="scrolling-wrapper">
                {' '.join(f'<div class="scrolling-content">{entry}</div>' for entry in duplicated_content)}
            </div>
        </div>
        """
        , unsafe_allow_html=True
...
# JavaScript for updating scrolling content (if necessary)
st.markdown(
    """
    <script>
    ...
    setInterval(function() {
        updateScrollContent();
    }, 30000); // Adjust refresh interval as needed (in milliseconds)
    </script>
    """
    , unsafe_allow_html=True)